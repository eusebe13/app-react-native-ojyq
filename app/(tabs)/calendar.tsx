...
export default function FirebaseCalendarScreen() {
  ...
  // Web-only helpers: format dates for HTML date/time inputs
          ...
          >
            ...
            <View style={dynamicStyles.row}>
              <View style={{ flex: 1, marginRight: 10 }}>
                ...
                {Platform.OS === "web" ? (
                  // @ts-ignore – web-only: direct click target opens browser picker natively
                  <input
                    type="date"
        ...
        <View style={dynamicStyles.modalView}>
          ...
          <ScrollView style={{ width: "100%" }}>
            ...
            {Platform.OS === "web" ? (
              ...
              <input
                type="time"
                value={toTimeInputValue(availabilityStartTime)}
                onChange={(e: any) => {
                  const val = e.target.value;
                  if (!val) return;
                  const [h, min] = val.split(":").map(Number);
                  const next = new Date(availabilityStartTime);
                  next.setHours(h, min);
                  setAvailabilityStartTime(next);
                }}
                style={{
                  width: "100%",
                  height: "45px",
            ...
            {Platform.OS === "web" ? (
              ...
              <input
                type="time"
                value={toTimeInputValue(availabilityEndTime)}
                onChange={(e: any) => {
                  const val = e.target.value;
                  if (!val) return;
                  const [h, min] = val.split(":").map(Number);
                  const next = new Date(availabilityEndTime);
                  next.setHours(h, min);
                  setAvailabilityEndTime(next);
                }}
                style={{
                  width: "100%",
                  height: "45px",
