...
export default function ChannelScreen(): ReactElement {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled && result.assets[0].base64) {
      await sendMessage("", `data:image/jpeg;base64,${result.assets[0].base64}`);
    }
  };
  ...
  const handleVote = async (msgId: string, poll: any, optionIndex: number) => {
    ...
    const hasVoted = poll.options.some((opt: any) => opt.voters.includes(currentUser._id));
    if (hasVoted) {
  ...
  const handleLongPress = (msg: ExtendedMessage) => {
    ...
    Alert.alert("Supprimer ?", "Ce message sera définitivement supprimé.", [
      { text: "Annuler", style: "cancel" },
      {
        text: "Supprimer",
        style: "destructive",
        onPress: async () => await deleteDoc(doc(db, "channels", id!, "messages", msg._id)),
      },
    ]);
  };
  ...
  const renderMessage = ({ item }: { item: ExtendedMessage }) => {
    ...
    const userPreset = item.user.avatarPreset !== undefined ? PRESET_AVATARS[item.user.avatarPreset] : null;
    ...
    return (
      ...
      >
        {!isMe && (
          <View style={[styles.avatar, userPreset ? { backgroundColor: userPreset.bg } : { backgroundColor: colors.primary }]}>
            {item.user.avatar ? (
        ...
        <View style={[styles.msgWrap, item.poll && { width: "85%", maxWidth: "100%" }]}>
          {!isMe && (
          ...
          >
            {item.image && (
              ...
              <TouchableOpacity onPress={() => setSelectedImage(item.image!)} activeOpacity={0.9}>
            ...
            {item.poll && (
              <View style={styles.pollContainer}>
                <Text style={[styles.pollQuestion, { color: isMe ? "#FFFFFF" : colors.textPrimary }]}>
                  📊 {item.poll.question}
                </Text>
                ...
                {item.poll.isActive ? (
                  <View style={{ width: '100%', gap: 8 }}>
                    ...
                    {item.poll.options?.map((opt: any, index: number) => (
                      <TouchableOpacity
                        key={index}
                        style={[styles.pollBtn, { backgroundColor: isMe ? "rgba(255,255,255,0.2)" : colors.primaryTint }]}
                        onPress={() => handleVote(item._id, item.poll!, index)}
                      >
                        <Text style={[styles.pollBtnText, { color: isMe ? "#FFFFFF" : colors.primary }]}>
                  ...
                  <Text style={{ color: isMe ? "rgba(255,255,255,0.7)" : colors.textTertiary, fontSize: tokens.font.xs, marginTop: 4 }}>
                    Sondage terminé
                  </Text>
                )}
              </View>
            ...
            {!!item.text && !item.poll && (
              <Text style={[styles.msgText, isMe ? { color: "#FFFFFF" } : { color: colors.textPrimary }]}>
                {item.text}
              </Text>
            )}
            
            <Text style={[styles.msgTime, isMe ? { color: "rgba(255,255,255,0.65)", textAlign: "right" } : { color: colors.textTertiary }]}>
              {time}
            </Text>
          </View>
        </View>
  ...
  return (
    <SafeAreaView style={[styles.flex1, { backgroundColor: colors.surfaceDim }]}>
      ...
      <Modal visible={pollModalVisible} transparent={true} animationType="slide" onRequestClose={() => setPollModalVisible(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.pollModalOverlay}>
          <View style={[styles.pollModalContent, { backgroundColor: colors.surface, maxHeight: '80%' }]}>
            <Text style={[styles.pollModalTitle, { color: colors.textPrimary }]}>Créer un sondage</Text>
            
            <TextInput
              style={[styles.pollModalInput, { borderColor: colors.border, color: colors.textPrimary, fontWeight: 'bold' }]}
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                  <TextInput
                    style={[styles.pollModalInput, { flex: 1, marginBottom: 0, borderColor: colors.border, color: colors.textPrimary }]}
            ...
            {pollOptions.length < 6 && (
              <TouchableOpacity 
                ...
                style={{ marginVertical: 15, paddingVertical: 10, alignItems: 'center', backgroundColor: colors.surfaceDim, borderRadius: 8 }}
              >
                <Text style={{ color: colors.primary, fontWeight: "bold" }}>+ Ajouter une option</Text>
              </TouchableOpacity>
            )}

            <View style={styles.pollModalButtons}>
              <TouchableOpacity style={styles.pollModalCancel} onPress={() => setPollModalVisible(false)}>
                <Text style={{ color: colors.textSecondary }}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.pollModalSubmit, { backgroundColor: colors.primary }]} onPress={submitPoll}>
...
const getStyles = (colors: any, tokens: any) =>
  StyleSheet.create({
    ...
    pollQuestion:  { fontWeight: "700", marginBottom: 15, fontSize: tokens.font.md, textAlign: "center" },
    pollButtons:   { flexDirection: "row", gap: tokens.space.md, width: "100%", justifyContent: "center" },
    pollBtn: { flex: 1, paddingVertical: tokens.space.md, borderRadius: 10, alignItems: "center" },
    pollBtnText: { color: "#FFFFFF", fontWeight: "700", fontSize: tokens.font.sm },
    ...
    inputAction: {
      ...
      marginRight: tokens.space.sm, marginBottom: 1,
    },
    textInput: {
      flex: 1, borderRadius: 20,
      paddingHorizontal: 14, paddingTop: 10, paddingBottom: 10,
      maxHeight: 120, lineHeight: 20, fontSize: tokens.font.md,
      backgroundColor: colors.surfaceDim, color: colors.textPrimary,
    },
    sendBtn: {
      width: 38, height: 38, borderRadius: 19,
      alignItems: "center", justifyContent: "center",
      marginLeft: tokens.space.sm, marginBottom: 1,
    },
    ...
    pollModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 },
    pollModalContent: { borderRadius: 16, padding: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
    pollModalTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 15 },
    pollModalInput: { borderWidth: 1, borderRadius: 8, padding: 12, fontSize: 16, marginBottom: 20 },
    pollModalButtons: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10 },
    pollModalCancel: { padding: 10 },
    pollModalSubmit: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },
  });